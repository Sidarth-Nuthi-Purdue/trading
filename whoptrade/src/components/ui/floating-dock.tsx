 ...
 * 
 * A modern, premium UI component that provides a floating navigation dock.
 * - Desktop: Positioned at the bottom with hover animations
 * - Mobile: Collapsible menu positioned at bottom right
 */
              ...
              >
                <Link
                  ...
                  className={cn(
                    ...
                    item.isActive && "ring-2 ring-blue-500"
                  )}
                >
                  <div className={cn(
                    "h-4 w-4",
                    item.isActive ? "text-blue-500" : "text-neutral-500 dark:text-neutral-400"
                  )}>
                    {item.icon}
                  </div>
                </Link>
...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "fixed bottom-6 left-1/2 z-50 hidden h-16 -translate-x-1/2 items-end gap-4 rounded-2xl bg-gray-50 px-4 pb-3 shadow-lg md:flex dark:bg-neutral-900",
        className,
      )}
    >
      {items.map((item) => (
